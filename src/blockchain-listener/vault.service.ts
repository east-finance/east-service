...
export class VaultService {
  ...
  async addVaultLog({txId, vault, address, sqlTx, vaultId} : {
    ...
    await (sqlTx || this.knex)(Tables.VaultLog).insert({
      ...
      west_amount_diff: oldVault ? (parseInt(vault.westAmount) || 0) - parseInt(oldVault.westAmount) : vault.westAmount,
      east_amount_diff: oldVault ? (parseInt(vault.eastAmount) || 0) - parseInt(oldVault.eastAmount) : vault.eastAmount,
      rwa_amount_diff: oldVault ? (parseInt(vault.rwaAmount) || 0) - parseInt(oldVault.rwaAmount) : vault.rwaAmount,
